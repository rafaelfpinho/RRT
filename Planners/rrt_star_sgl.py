...
def main():
	...
	for i in range(NUMNODES):
		...
		if dist(bestNode.get_point(),goalPoint) < RADIUS and (fim-init)>execution_time:
			...
			arquivo = open('/home/bob/drone_arch/Data/route_gustavo.txt','w')
			...
			while bestNode.get_parent() != None:
				...
				if graph:
					if aux != None:
						pygame.draw.line(screen,red,( int(bestNode.get_x()),int(bestNode.get_y()) ),(int(aux.get_x()),int(aux.get_y()))  )
						pygame.display.update()
